...
void draw(
	...
	Coord window_size)
{
	for (int y = cursor_coord.Y - radius + 1; y <= cursor_coord.Y + radius - 1 && y < canvas.size() - 1; y++)
	{
		for (int x = cursor_coord.X - radius + 1; x <= cursor_coord.X + radius - 1 && x < canvas[y].size() - 1; x++)
		{
